...
public class Zscore{
    ...
    public static List<ZscoreValue> callZscore() throws Exception { // Calculation of z score
        double zScore = 0.0;
        List<ZscoreValue> list = new ArrayList<ZscoreValue>();
        try {
            int sum = 0;
            ...
            for (int i = 0; i < WordNChar.characters.size(); i++) {
                zScore = (WordNChar.characters.get(i).getNumber()-mean)/standardDeviation.get(); // Calculate every alphabet's z score
                list.add(new ZscoreValue(zScore)); // Assign z score value into list from ZscoreValue.java
            }
        }catch(Exception e){
            e.printStackTrace();
        }
    ...
    public static void ZscoreResult(){ // Print all z score result
        try {
            int count = 1;
            System.out.println("*** Z score value for alphabet: ***");
